...
佑哉さんと祐希ちゃんの「行きたいところ」「やりたいこと」をメモする PWA。

- **今**：佑哉さん一人で使える（端末内に保存）
- **将来**：祐希ちゃんと共有（JSON 送受信 or クラウド同期）
...
`main` ブランチへの push で自動デプロイ（`.github/workflows/deploy-pages.yml`）。

Settings → Pages → Source: **GitHub Actions** を選択。

公開 URL 例：`https://kikurin-tasogare.github.io/futari-list-app-/`
...
3. 祐希ちゃん：同じアプリを開く → **インポート**（マージ ON）
4. 祐希ちゃんは共有タブで名前を「祐希」に変更

### 方法B：クラウド同期（リアルタイム）

1. [Supabase](https://supabase.com) で無料プロジェクト作成
2. SQL Editor で以下を実行
3. 佑哉さん・祐希ちゃん両方のアプリ「共有」タブに URL / Anon Key を入力
4. 佑哉さんの **招待コード** を祐希ちゃんに送る → 祐希ちゃんは「参加」
